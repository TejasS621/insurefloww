"""
Handle payment routes for the provider backend.

Args:
    None: This module defines broker-authenticated payment session endpoints
    plus the hosted mock checkout page used during local integration testing.

Returns:
    None: Route handlers return structured payment responses or HTML content
    under the provider payment routers.
...
from backend.provider_backend.core.apis.schemas.requests.payment_request import (
...
from backend.provider_backend.core.apis.schemas.responses.payment_response import (
...
async def create_payment_session(
    ...
    """
    Create a provider-owned payment session for a broker-authenticated request.

    Args:
        request_data: Validated payment-session payload supplied by the broker.
        engine: Active ODMantic database engine dependency.
        _: Authenticated broker dependency used to enforce trusted access.

    Returns:
        APIResponse[ProviderPaymentResponse]: Persisted provider payment
        session details for downstream orchestration.
    ...
    try:
        ...
        logger.exception("Failed to create provider payment session.")
        raise
...
async def create_mock_payment_session(
    ...
    """
    Create a mock hosted payment session for a broker-authenticated request.

    Args:
        request_data: Validated hosted-payment payload supplied by the broker.
        engine: Active ODMantic database engine dependency.
        _: Authenticated broker dependency used to enforce trusted access.

    Returns:
        APIResponse[MockPaymentSessionResponse]: Redirect-friendly hosted
        payment session details for frontend checkout flows.
    ...
    try:
        ...
        logger.exception("Failed to create mock payment session.")
        raise
...
async def render_mock_payment_page(
    ...
    """
    Render the hosted mock payment page used for local checkout simulation.

    Args:
        payment_reference: Provider payment reference used to render checkout.
        engine: Active ODMantic database engine dependency.

    Returns:
        HTMLResponse: Rendered mock checkout page with available payment methods.
    ...
    try:
        ...
        if payment is None:
            raise NotFoundServiceError(
                "Mock payment page not found for the supplied payment reference."
            )
        ...
        logger.exception("Failed to render mock payment page for %s.", payment_reference)
        raise
