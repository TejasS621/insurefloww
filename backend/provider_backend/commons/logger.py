"""
Provide centralized logger configuration for the provider backend.

Args:
    None: This module defines helper functions for building named loggers with
    consistent console and file handlers for the provider backend.
...
LOG_DIRECTORY = Path("logs") / "provider_backend"
LOG_FORMAT = (
...
def _build_formatter() -> logging.Formatter:
    """Return the shared formatter used by provider backend log handlers."""
    return logging.Formatter(LOG_FORMAT)


def configure_logger(logger_name: str, *, level: int = logging.INFO) -> logging.Logger:
    """
    Configure and return a named logger for the provider backend.
...
def get_logger(name: str) -> logging.Logger:
    """
    Return a configured logger for the provider backend.
