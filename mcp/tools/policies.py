...
from mcp.schemas.policies import DownloadPolicyInput, DownloadPolicyOutput, GetPolicyInput, PolicyOutput
...
class PolicyTools:
    ...
    async def get_policy(self, payload: GetPolicyInput) -> ToolResult[PolicyOutput]:
        ...
        if not self.settings.customer_jwt_token:
            return error_result(
                AuthenticationRequiredError("get_policy requires CUSTOMER_JWT_TOKEN to access customer policy data.")
            )

        try:
            response = await self.main_client.get_policy(payload.policy_number, self.settings.customer_jwt_token)
    ...
    async def download_policy(self, payload: DownloadPolicyInput) -> ToolResult[DownloadPolicyOutput]:
        ...
        if not self.settings.customer_jwt_token:
            return error_result(
                AuthenticationRequiredError("download_policy requires CUSTOMER_JWT_TOKEN to access policy files.")
            )
        ...
        try:
            ...
            file_path = await self.main_client.download_policy(
                ...
                token=self.settings.customer_jwt_token,
